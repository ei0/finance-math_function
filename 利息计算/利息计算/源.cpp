...
int default_val_curr_choice(){//金额货币种类修改
	int ch_curr;
	cout << "金额可选择的货币种类：人民币(默认)、港元、美元、欧元、英镑、日元、韩元/" << endl;
	cin >> ch_curr;
...
int default_val_size_choice(){//金额大小修改
	cout << "金额可选择的单位大小：元(默认)、千元、万元" << endl;
	int ch_size;
	cin >> ch_size;
...
void parameter_input(){//参数输入
	...
	if (choice == 1){
		...
		cout << "请输入您想要计算的金额_____，年份_____以及利息率_____" << endl;
		cin >> val >> t >> inr;
		inr = inr*0.01;
	}
}
double deferred_annuity_pre(){
	...
	cout << "请输入不需还本付息的年数"<< endl;
	cin >> temp_t1;
	cout << "请输入偿还本息的年数" << endl;
	cin >> temp_t2;
	cout <<"请输入利息率" << endl;
